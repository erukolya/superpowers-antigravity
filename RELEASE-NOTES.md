...
* **SDD: resume-based fix loop and plan-scoped ledger.** Fix rounds resume the idle implementer instead of dispatching a fresh one — no context rebuild per round — and every round ends with a scoped re-review that verifies the fixes rather than re-reading the whole task, capped at five rounds per task before the controller adjudicates. Progress ledgers are now per-plan (`.superpowers/sdd/<plan>/`), actually git-ignored, and deleted once the final review is clean. Upstream implements this with helper scripts and brief/report files, which branch-isolated implementers can't read; the same outcomes are implemented natively in the skill instead.

### Antigravity 2.0 API Corrections

* `invoke_subagent` examples use the `Subagents` array form everywhere — the platform has no bare named-parameter call.
* Artifact typing and `RequestFeedback` moved inside `ArtifactMetadata`; `write_to_file` has no such top-level arguments. Five call sites corrected.
* Model tiers live in agent-file frontmatter (`model: flash|pro|inherit`); neither `define_subagent` nor `invoke_subagent` takes a model argument. SDD's model-selection guidance now points at the real mechanism.
* `send_message` reaches idle subagents — they re-awaken with their full context. Fresh dispatch is only needed for killed or errored agents.
* Removed undocumented behavior claims: `schedule` timers don't auto-cancel when a subagent replies, background tasks don't push completion notifications (poll `manage_task` status on a `schedule` cadence), and `Workspace` has no default value.
* `/browser` is the sole entry point to the sandboxed browser subagent and adds no browser tools to the calling session — `browser-testing` now says so.

### New

* **Bundled subagents.** The four SDD roles — `implementer`, `spec-reviewer`, `code-reviewer`, `re-reviewer` — ship as plugin agents with read-only tool lists for the reviewers. Dispatch them by TypeName; no per-session `define_subagent` setup. A new static lint (`tests/antigravity/test-agent-frontmatter.sh`) validates every agent's frontmatter and tool names against the documented tool set.
* **Native rule bootstrap.** `using-superpowers` also ships as a rule (`rules/using-superpowers.md`), so the methodology can load through the plugin's rule channel: set it to Always On in the Rules panel for automatic loading in every session, or load it on demand with `/using-superpowers`. The existing context-file channel is unchanged. A sync test keeps the rule byte-identical to the skill.
* **Edit-time purity gate.** A `PreToolUse` hook (`hooks.json` + `hooks/purity-check.sh`) blocks writes that would reintroduce banned vocabulary into this plugin's own `skills/` tree, enforcing the same patterns as the purity test before the write lands. Strictly scoped: writes in other projects are never touched, and ambiguous paths always pass.
* **SDD error-state handling and comment-driven review.** A subagent in the terminal error state is dispatched fresh rather than resumed, and inline artifact comments on plans and design docs are treated as change requests to resolve, not just chat replies.

### Fixes

* **`writing-skills` taught the wrong home for new skills.** Personal skills go in `~/.gemini/config/skills/` (global) or `.agents/skills/` (workspace) — not the plugin install directory. The frontmatter guidance now matches the platform: `description` required, `name` optional (lowercase, defaults to the folder name), and the 1024-character cap is house style, not a platform limit.
* **The headless test suite can actually run.** `agy` invocations now pass `--print-timeout` (headless runs otherwise self-terminate at the 5-minute default) and `--dangerously-skip-permissions` (the default mode pauses for interactive diff review and hangs unattended runs). Skill-trigger detection reads `stream-json` tool events instead of grepping response prose, transcript lookup resolves the workspace's conversation from the CLI's `last_conversations.json` instead of racing on file timestamps, and `AGY_TEST_MODEL` / `AGY_TEST_EFFORT` pin the model and effort for reproducible runs.
* **README installation covers the CLI.** The CLI keeps its own plugin store: `agy plugin install <path>` installs there, separate from the IDE's plugin directories. Verify Installation lists the working checks, `/using-superpowers` included.

### Not Ported from Upstream v6.2.0

* **Windows SessionStart hook fix** — this fork has no SessionStart hook; its bootstrap channels are line-ending-safe already.
* **Gemini CLI restoration, Codex packaging fix, mapping-test scoping** — they patch multi-platform infrastructure this fork removed in v6.0.0.
...
* **New: `test-skill-tool-purity.sh`** — Static analysis test that verifies zero legacy tool names, zero platform references, and zero tool mapping references across all skill files. Runs without the `agy` binary.
...
All 12 feature gaps identified in the gap analysis have been implemented across the skill library to maximize token savings, verification quality, and native capability usage:

* **Lightweight Research Subagents** — `TypeName: "research"` subagents for read-only codebase exploration and code reviewing, minimizing context size and API cost (`brainstorming`, `requesting-code-review`).
* **Interactive User Feedback** — `RequestFeedback: true` in `ArtifactMetadata` for design documents, implementation plans, and walkthroughs, enabling structured feedback loops (`brainstorming`, `writing-plans`, `executing-plans`).
* **Workspace Mode Selection** — "Workspace Mode Decision Table" guiding choice of `branch` (isolated write), `inherit` (read-only), or `share` (parallel non-overlapping edits) (`using-git-worktrees`, `subagent-driven-development`, `dispatching-parallel-agents`, `requesting-code-review`).
* **Native Web Research** — `search\_web` and `read\_url\_content` for troubleshooting unfamiliar errors or retrieving external API docs (`systematic-debugging`, `brainstorming`, `writing-plans`).
* **Native Search Tools** — `list\_dir` and `grep\_search` by name instead of generic grep/find patterns (`brainstorming`, `writing-plans`, `systematic-debugging`).
* **Slash Command Recommendations** — `/grill-me`, `/goal`, and `/browser` recommendations for interactive alignment, autonomous sessions, or browser automation (`brainstorming`, `browser-testing`).
* **Subagent Monitoring** — `manage\_subagents` (`Action: "list"` and `"kill"`) to verify subagent statuses or clean up stalled processes (`subagent-driven-development`, `dispatching-parallel-agents`).
* **Recurring Cron Verification** — `schedule(CronExpression: ..., MaxIterations: ...)` for long-running builds or test suites (>5 minutes) (`verification-before-completion`, `subagent-driven-development`).
* **Sandboxed Permissions** — `ask\_permission` for destructive operations like branch discarding (`finishing-a-development-branch`).
* **Targeted File Edits** — `replace\_file\_content` and `multi\_replace\_file\_content` instead of `write\_to\_file` for existing skill modifications (`writing-skills`).
* **Rich Markdown Formatting** — Mermaid flowcharts, carousels, and severity-classified GitHub alerts (`\[!CAUTION]`, `\[!WARNING]`, `\[!NOTE]`, `\[!TIP]`) for structured outputs (`systematic-debugging`, `requesting-code-review`, `browser-testing`).

#### Documentation

* **README.md** — Added fork attribution, "What's different from upstream?" comparison table, Activation section, migration instructions, and updated Verify Installation steps.
